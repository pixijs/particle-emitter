 ...
 * Compiled Fri, 01 Mar 2019 03:13:06 UTC
 *
 * pixi-particles is licensed under the MIT License.
 * http://www.opensource.org/licenses/mit-license
 */
this.PIXI = this.PIXI || {};
this.PIXI.particles = (function (exports, pixi_js) {
	'use strict';
	...
	var ParticleUtils = {
	    ...
	    getBlendMode: function (name) {
	        if (!name)
	            return pixi_js.BLEND_MODES.NORMAL;
	        name = name.toUpperCase();
	        while (name.indexOf(" ") >= 0)
	            name = name.replace(" ", "_");
	        return pixi_js.BLEND_MODES[name] || pixi_js.BLEND_MODES.NORMAL;
	    },
	    /**
	...
	***************************************************************************** */
	...
	var Particle = /** @class */ (function (_super) {
	    ...
	    function Particle(emitter) {
	        ...
	        _this.velocity = new pixi_js.Point();
	        _this.maxLife = 0;
	        _this.age = 0;
	        _this.ease = null;
	         ...
	         */
	        _this.acceleration = new pixi_js.Point();
	        /**
	    ...
	    Particle.prototype.applyArt = function (art) {
	        this.texture = art || pixi_js.Texture.EMPTY;
	    };
	    /**
	    ...
	    Particle.parseArt = function (art) {
	        ...
	        for (i = art.length; i >= 0; --i) {
	            if (typeof art[i] == "string")
	                art[i] = pixi_js.Texture.fromImage(art[i]);
	        }
	...
	var ticker = pixi_js.ticker.shared;
	var helperPoint = new pixi_js.Point();
	/**
	 * A particle emitter.
	 * @memberof PIXI.particles
	 * @class Emitter
	...
	var Emitter = /** @class */ (function () {
	    ...
	    Emitter.prototype.init = function (art, config) {
	        ...
	        if (acceleration && (acceleration.x || acceleration.y)) {
	            ...
	            this.acceleration = new pixi_js.Point(acceleration.x, acceleration.y);
	            this.maxSpeed = config.maxSpeed || NaN;
	        }
	        else
	            this.acceleration = new pixi_js.Point();
	        //set up the scale
	        if (config.scale) {
	        ...
	        switch (config.spawnType) {
	            case "rect":
	                ...
	                this.spawnRect = new pixi_js.Rectangle(spawnRect.x, spawnRect.y, spawnRect.w, spawnRect.h);
	                break;
	            case "circle":
	                this.spawnType = "circle";
	                this._spawnFunc = this._spawnCircle;
	                spawnCircle = config.spawnCircle;
	                this.spawnCircle = new pixi_js.Circle(spawnCircle.x, spawnCircle.y, spawnCircle.r);
	                break;
	            case "ring":
	                this.spawnType = "ring";
	        ...
	        this.spawnPos = new pixi_js.Point(config.pos.x, config.pos.y);
	    ...
	    Emitter.prototype.update = function (delta) {
	        if (this._autoUpdate) {
	            delta = delta / pixi_js.settings.TARGET_FPMS / 1000;
	        }
	 ...
	 * @private
	 */
	var helperPoint$1 = new pixi_js.Point();
	...
	var PathParticle = /** @class */ (function (_super) {
	    ...
	    function PathParticle(emitter) {
	        ...
	        _this.initialPosition = new pixi_js.Point();
	        _this.movement = 0;
	        return _this;
	    }
	    /**
	...
	var AnimatedParticle = /** @class */ (function (_super) {
	    ...
	    AnimatedParticle.prototype.update = function (delta) {
	        ...
	        if (lerp >= 0) {
	            ...
	            this.texture = this.textures[frame] || pixi_js.Texture.EMPTY;
	        }
	        return lerp;
	    };
	    /**
	        ...
	        for (var i = 0; i < art.length; ++i) {
	            ...
	            for (var j = 0; j < textures.length; ++j) {
	                ...
	                if (typeof tex == "string")
	                    outTextures.push(pixi_js.Texture.fromImage(tex));
	                else if (tex instanceof pixi_js.Texture)
	                    outTextures.push(tex);
	                else {
	                    var dupe = tex.count || 1;
	                    if (typeof tex.texture == "string")
	                        tex = pixi_js.Texture.fromImage(tex.texture);
	                    else
	                        tex = tex.texture;
	                    for (; dupe > 0; --dupe) {
	                        outTextures.push(tex);
	...
	var ParticleRenderer = pixi_js.particles.ParticleRenderer;
	var ParticleContainer = pixi_js.particles.ParticleContainer;
